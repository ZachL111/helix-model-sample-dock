...
`helix-model-sample-dock` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for sample workflows, centered on diagnostic reporting, negative fixtures, and human-readable error snapshots.
...
The score is deterministic. The current threshold is 178. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 3.
